...
def embed_and_upsert(chunks: List[Dict], batch_size: int = 64):
    try:
        print(f"[DEBUG] Starting embed_and_upsert with {len(chunks)} chunks, batch_size={batch_size}")
        
        for start in range(0, len(chunks), batch_size):
            batch = chunks[start:start+batch_size]
            print(f"[DEBUG] Processing batch {start//batch_size + 1}/{(len(chunks) + batch_size - 1)//batch_size} with {len(batch)} chunks")
            
            # Generate embeddings
            print(f"[DEBUG] Generating embeddings for batch...")
            ...
            print(f"[DEBUG] Prepared {len(upsert_items)} upsert items")
            
            # Upsert to Pinecone
            print(f"[DEBUG] Upserting to Pinecone...")
            index.upsert(vectors=upsert_items)
            print(f"[DEBUG] Successfully upserted batch to Pinecone")
            
        print(f"[DEBUG] Completed embed_and_upsert for all {len(chunks)} chunks")
        
    except Exception as e:
        print(f"[ERROR] Error in embed_and_upsert: {str(e)}")
...
def process_and_store(text: str, user_id: str, source: str, document_id: str) -> int:
    try:
        print(f"[DEBUG] Processing text for user {user_id}, document {document_id}")
        ...
        print(f"[DEBUG] Starting embedding and upsert process...")
        embed_and_upsert(chunks)
        print(f"[DEBUG] Successfully embedded and upserted {len(chunks)} chunks")
        
        return len(chunks)
    except Exception as e:
        print(f"[ERROR] Error in process_and_store: {str(e)}")
        print(f"[ERROR] Exception type: {type(e).__name__}")
        import traceback
        print(f"[ERROR] Traceback: {traceback.format_exc()}")
        raise
