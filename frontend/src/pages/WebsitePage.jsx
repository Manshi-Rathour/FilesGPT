...
export default function WebsitePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("http://127.0.0.1:5000/website/upload/", {
        method: "POST",
        body: formData,
        headers: {
          Authorization: `Bearer ${token}` // ✅ Use JWT Bearer token authentication
        }
      });
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      ...
      {/* 🔷 Main Content */}
      <div className="flex items-center justify-center h-screen p-6">
        <div className="bg-black/50 backdrop-blur-lg rounded-3xl shadow-lg p-6 w-full max-w-md flex flex-col gap-4 border border-gray-700">
          <h2 className="font-bold text-white text-center text-xl mb-2">
            Enter Website URL
          </h2>
          ...
          <input
            ...
            value={url}
            onChange={(e) => setUrl(e.target.value)}
            className="px-3 py-2 rounded-lg bg-black/20 text-white placeholder-gray-400 border border-gray-600 focus:border-green-500 outline-none transition"
          />
          ...
          <button
            onClick={goBack}
            className="w-full mt-2 text-white border border-green-500 px-4 py-2 rounded-lg hover:bg-green-700 transition font-medium"
          >
            Back
          </button>
        </div>
